...
public:
    ...
    EnergyEvseCluster(const Config & config) :
        ...
        mOptionalAttrs(config.optionalAttrs), mOptionalCmds(config.optionalCmds), mFeatureAttrs([&]() {
            BitMask<EnergyEvse::FeatureAttributes> attrs;
            if (config.feature.Has(EnergyEvse::Feature::kV2x))
            {
                attrs.Set(FeatureAttributes::kDischargingEnabledUntil);
                attrs.Set(FeatureAttributes::kMaximumDischargeCurrent);
                attrs.Set(FeatureAttributes::kSessionEnergyDischarged);
            }
            if (config.feature.Has(EnergyEvse::Feature::kChargingPreferences))
            {
                attrs.Set(FeatureAttributes::kNextChargeStartTime);
                attrs.Set(FeatureAttributes::kNextChargeTargetTime);
                attrs.Set(FeatureAttributes::kNextChargeRequiredEnergy);
                attrs.Set(FeatureAttributes::kNextChargeTargetSoC);
                if (config.optionalAttrs.Has(OptionalAttributes::kSupportsApproximateEvEfficiency))
                {
                    attrs.Set(FeatureAttributes::kApproximateEvEfficiency);
                }
            }
            if (config.feature.Has(EnergyEvse::Feature::kSoCReporting))
            {
                attrs.Set(FeatureAttributes::kStateOfCharge);
                attrs.Set(FeatureAttributes::kBatteryCapacity);
            }
            if (config.feature.Has(EnergyEvse::Feature::kPlugAndCharge))
            {
                attrs.Set(FeatureAttributes::kVehicleID);
            }
            return attrs;
        }())
    {
    ...
    const BitMask<EnergyEvse::FeatureAttributes> & FeatureAttrs() const { return mFeatureAttrs; }
...
private:
    ...
    const BitMask<EnergyEvse::FeatureAttributes> mFeatureAttrs;
};
